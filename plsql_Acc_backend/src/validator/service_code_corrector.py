...
class ServiceCodeValidator:
    ...
    def _validate_validation_logic(self, body: str, extracted_logic: Dict[str, Any]):
        ...
        BUG-2c FIX — validator only checked throw counts, not whether mutations
        (setStatus(), save()) actually appeared in the method body.  A partial LLM
        response that omits the mutation code but includes the right number of
        throws passed validation unchallenged.

        New checks added:
          1. If the extracted logic contains INSERT/UPDATE/DELETE, at least one
             repository.save() / deleteById() call must be present in the body.
          2. If the extracted logic contains UPDATE assignments, at least one
             entity setter (existing.setXxx) must be present.
        """
        error_assertions = extracted_logic.get('error_assertions', [])
        validations = extracted_logic.get('validations', [])

        # ── Original check: throw count ──────────────────────────────────────
        ...
        # ── BUG-2c FIX: mutation presence checks ────────────────────────────
        inserts  = extracted_logic.get('inserts',  [])
        updates  = extracted_logic.get('updates',  [])
        deletes  = extracted_logic.get('deletes',  [])

        has_dml = bool(inserts or updates or deletes)
        has_save_call     = bool(re.search(r'\.save\s*\(', body))
        has_delete_call   = bool(re.search(r'\.delete\w*\s*\(', body))
        has_any_mutation  = has_save_call or has_delete_call

        if has_dml and not has_any_mutation:
            self.issues.append(ValidationIssue(
                severity='error',
                category='logic',
                location='database operations layer',
                message=(
                    f'Extracted logic has {len(inserts)} INSERT(s), {len(updates)} UPDATE(s), '
                    f'{len(deletes)} DELETE(s) but no repository.save() / deleteById() found in body'
                ),
                suggestion=(
                    'The method body is missing its persistence calls. '
                    'Re-generate or manually add repository.save(entity) / repository.deleteById(id).'
                ),
                code_snippet=None
            ))

        if updates:
            setter_count = len(re.findall(r'\.set[A-Z]\w*\s*\(', body))
            expected_setters = sum(len(u.get('assignments', {})) for u in updates)
            if expected_setters > 0 and setter_count == 0:
                self.issues.append(ValidationIssue(
                    severity='error',
                    category='logic',
                    location='update assignments',
                    message=(
                        f'UPDATE has {expected_setters} assignment(s) but no entity.setXxx() '
                        f'calls found in body'
                    ),
                    suggestion=(
                        'Entity field setters are missing. The UPDATE block was not translated. '
                        'Check that _extract_updates found the correct SET clause.'
                    ),
                    code_snippet=None
                ))
...
class ServiceCodeCorrector:
    ...
    def correct_plsql_syntax(code: str) -> str:
        """
        Remove PL/SQL syntax and convert to valid Java.

        BUG-2a FIX — named-parameter converter produced Kotlin/Python syntax:
            The old regexes turned `key = value,` into `key: value,` which is
            still invalid Java.  Named-parameter syntax (`=>` and positional `=`)
            should have been eliminated *in the converter pipeline* before code
            reaches this post-processor.  Here we simply DELETE the `key =`
            prefix so only the value expression remains, which is correct for a
            positional Java call.

        BUG-2b FIX — single-quote converter skipped too many lines:
            The guard `if 'SELECT' in line or 'WHERE' in line` is both too
            broad (skips ordinary Java lines that happen to contain those words)
            and too narrow (misses other SQL keywords).  The correct approach is
            to convert single-quoted string *literals* everywhere – a real SQL
            query wouldn't survive as a bare single-quoted string in Java anyway.
        """
        # ── Step 1: PL/SQL assignment operator -> Java assignment ──────────────
        # Protect existing == comparisons before replacing := 
        code = code.replace('==', '__DBLEQ__')
        code = code.replace(':=', '=')
        code = code.replace('__DBLEQ__', '==')

        # ── Step 2: Single-quoted strings -> double-quoted Java strings ─────────
        # BUG-2b FIX: apply to ALL lines.  We only skip lines that contain a
        # raw SQL fragment (prefixed with a repository/JPA keyword that implies
        # the string will stay as SQL), and comment lines.
        sql_line_keywords = ('SELECT ', 'UPDATE ', 'DELETE ', 'INSERT ', 'FROM ', 'JOIN ', 'WHERE ', 'ORDER BY', 'GROUP BY')
        lines = []
        for line in code.split('\n'):
            stripped = line.lstrip()
            is_sql_line = any(stripped.upper().startswith(kw) for kw in sql_line_keywords)
            is_comment  = stripped.startswith('//')
            if is_sql_line or is_comment:
                lines.append(line)
            else:
                # Replace 'text' -> "text" but leave already-double-quoted strings alone
                line = re.sub(r"(?<!\')\'([^\']*)\'", r'"\1"', line)
                lines.append(line)
        code = '\n'.join(lines)

        # ── Step 3: PL/SQL named-parameter (=>) already handled upstream ───────
        # If any stray => survived, strip the key= prefix so only the value
        # expression remains (valid Java positional argument).
        # BUG-2a FIX: do NOT convert to `key: value` (that's Kotlin/Python).
        # Pattern: word = word  inside an argument list  -> just the value word
        # Only apply inside argument-list context (preceded by '(' or ',').
        code = re.sub(
            r'(?<=[(,])\s*(\w+)\s*=>\s*',
            '',
            code
        ...
        # ── Step 4: Remove package-qualified references ──────────────────────────
        # Convert remaining package.function refs to bare function names only when
        # they were not already translated to service calls.
        code = re.sub(r'\b(\w+)_pkg\.(\w+)', r'\2', code)
