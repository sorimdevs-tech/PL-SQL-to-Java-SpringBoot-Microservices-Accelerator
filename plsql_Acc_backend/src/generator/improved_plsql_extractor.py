...
class ImprovedPLSQLExtractor:
    ...
    def extract_all_logic(proc_body: str) -> ExtractedLogic:
        ...
        # BUG-3 FIX: Extract "lookup-then-compute" patterns.
        # This is the most common business-logic pattern: SELECT a rate/factor
        # from a table, then multiply/divide by it.  The extractor had no concept
        # of this chain so when no arithmetic elements were produced the body
        # generator emitted nothing.  We now detect the chain and synthesise
        # a calculation entry so the generator has something to emit.
        ImprovedPLSQLExtractor._extract_lookup_compute_chains(body, logic)
    ...
    def _extract_calculations(body: str, logic: ExtractedLogic):
        ...
        """
        # Match: l_/v_/p_ variable := expression;  (BUG-3 FIX: was [lv]_ only, missed p_ accumulators)
        pattern = r'([lv]_\w+|p_\w+)\s*:=\s*([^;]+);'
...
def extract_full_procedure_body(source: str, proc_name: str) -> Optional[str]:
    ...
    # ──────────────────────────────────────────────────────────────────────────
    # BUG-3 FIX: Lookup-then-compute pattern extraction
    # ──────────────────────────────────────────────────────────────────────────

    @staticmethod
    def _extract_lookup_compute_chains(body: str, logic: 'ExtractedLogic') -> None:
        """
        BUG-3 FIX: Detect "SELECT rate INTO l_rate … l_total := amount * l_rate" chains.

        The original extractor only captured arithmetic in _extract_calculations,
        but _extract_calculations only matched   l_var / v_var := expr   (variables
        starting with l_ or v_).  It completely missed:
          a) assignments to p_ parameters used as accumulators
          b) multi-step "lookup + arithmetic" patterns where a SELECT feeds a
             subsequent multiplication / division

        Strategy
        ─────────
        1. For every variable that was loaded by a SELECT … INTO (i.e. every
           `into_variables` value in logic.selects) check whether that variable
           subsequently appears in any arithmetic expression in the body.
        2. If it does, synthesise a `calculation` entry that carries the full
           arithmetic expression so the code generator can emit it.
        3. Also widen _extract_calculations to cover p_ prefixed variables
           (done inline below as a second pass).
        """
        import re

        # ── Pass A: widen calculations to include p_ prefixed variables ──────
        # Original pattern only matched l_ and v_ — this adds p_ prefix too.
        p_pattern = re.compile(r'(p_\w+)\s*:=\s*([^;]+);', re.IGNORECASE)
        existing_vars = {c['variable'] for c in logic.calculations}
        for m in p_pattern.finditer(body):
            var = m.group(1).strip()
            expr = m.group(2).strip()
            if var not in existing_vars:
                logic.calculations.append({'variable': var, 'expression': expr})
                existing_vars.add(var)

        # ── Pass B: lookup-then-compute chain detection ───────────────────────
        if not logic.selects:
            return

        # Collect all variables that were populated by SELECT … INTO
        lookup_vars: set = set()
        for sel in logic.selects:
            into_raw = sel.get('into_variables', '')
            if not into_raw:
                continue
            # into_variables may be comma-separated (multiple INTO targets)
            for v in re.split(r'[,\s]+', into_raw):
                v = v.strip()
                if v:
                    lookup_vars.add(v.lower())

        if not lookup_vars:
            return

        # For each lookup variable find arithmetic expressions that reference it
        # Pattern: any assignment  xxx := <expr containing lookup_var>
        assign_pattern = re.compile(r'([\w]+)\s*:=\s*([^;]+);', re.IGNORECASE)
        existing_vars = {c['variable'].lower() for c in logic.calculations}

        for m in assign_pattern.finditer(body):
            target_var = m.group(1).strip()
            expr       = m.group(2).strip()

            # Check whether any lookup variable appears in this expression
            expr_lower = expr.lower()
            uses_lookup = any(lv in expr_lower for lv in lookup_vars)
            if not uses_lookup:
                continue

            # Check that the expression contains at least one arithmetic operator
            has_arithmetic = bool(re.search(r'[+\-*/]', expr))
            if not has_arithmetic:
                continue

            # Avoid duplicates
            if target_var.lower() in existing_vars:
                continue

            # Synthesise a calculation entry tagged with its source SELECT table
            # so the code generator can emit the right Java expression.
            source_table = ''
            for sel in logic.selects:
                into_raw = sel.get('into_variables', '')
                for lv in lookup_vars:
                    if lv in into_raw.lower() and lv in expr_lower:
                        source_table = sel.get('table', '')
                        break
                if source_table:
                    break
            ...
            logic.calculations.append({
                'variable'    : target_var,
                'expression'  : expr,
                'lookup_chain': True,          # flag for downstream use
                'source_table': source_table,
            })
            existing_vars.add(target_var.lower())

            logger.debug(
                f"[BUG-3-FIX] Lookup-compute chain detected: "
                f"{target_var} := {expr[:60]} (source_table={source_table})"
            )
