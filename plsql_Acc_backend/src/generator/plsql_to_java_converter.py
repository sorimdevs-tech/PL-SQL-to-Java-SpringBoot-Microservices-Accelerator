...
class PLSQLtoJavaConverter:
    ...
    def generate_java_method(
        ...
        if logic.return_type:
            ...
            # Handle %rowtype - return entity type
            if '%rowtype' in plsql_type.lower():
                # Extract table name from rowtype reference (e.g., xy_customer%rowtype -> XyCustomer)
                table_name = plsql_type.split('%')[0].strip().lower()
                return_type = PLSQLtoJavaConverter._table_to_entity(table_name)
            else:
                # Map PL/SQL type directly
