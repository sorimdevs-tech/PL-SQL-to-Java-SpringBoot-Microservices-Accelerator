...
def test_dynamic_logic_injection():
    ...
    print(f"✓ Extracted {len(logic.validations)} validations:")
    for val in logic.validations:
        print(f"  - {val['field']}: {val['condition']}")
    print(f"✓ Extracted {len(logic.inserts)} INSERT statements")
    print(f"✓ Transaction detected: {logic.has_commit}")
    
    # Test 2: Generate Java method with@Transactional
    ...
    checks = [
        ...
        ('customerId' in java_method or 'customer' in java_method.lower(), "Parameters included"),
...
public class InvoiceApiPkgCreateInvoiceService {
    ...
    print(f"✓ Found {len(matches)} method(s) to replace")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\n" + "=" * 60)
        if success:
            print("✅ ALL TESTS PASSED - Dynamic logic injection is working!")
            sys.exit(0)
        else:
